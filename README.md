...
3. 대기열에 새로운 사용자 등록, 삭제, 스케줄러에 의해 주기적으로 대기열의 사ㅛ


5. 스케줄러가 주기적으로 대기열의 사용자를 참가열로 이동시키며, outbox 테이블의 이동한 사용자 상태 값을 "ALLOW"로 수정합니다.
6. Debezium이 outbox 테이블의 변경사항을 실시간으로 감지하여 Kafka에 이벤트를 발행하고, 서버는 Kafka 메시지를 수신한 뒤, SSE를 통해 클라이언트로 전달합니다.
7. 이 이벤트를 수신한 각 사용자는 자신의 현재 상태를 서버로부터 받음, 아직 대기열에 있는 사용자는 현재 순위를 실시간으로 전달받고, 참가열로 이동된 사용자는 "confirmed" 이벤트를 받아 타겟 페이지로 이동하게 되는 구조
