...
이 시스템은 동시에 처리할 수 있는 요청의 수를 제한하고, 초과된 요청은 대기열 ( Queue )에 저장한 뒤 순차적으로 처리함으로써 서버의 과부하를 방지합니다.
...
이를 통해, 동시다발적으로 유입되는 대량의 트래픽 속에서도 접속자 수를 효과적으로 제어하고 관리할 수 있는 구조를 구성합니다.

대기열 시스템이 없는 구조에서는 사용자가 웹 애플리케이션에 직접 접근하여 곧바로 타겟 페이지의 정보를 요청하고 응답을 받는 구조였지만 <br>
, 접속자 대기열 시스템을 도입한 이후에는 사용자가 먼저 대기열 페이지로 진입하여 현재 대기 인원 수나 예상 대기 시간 등의 정보를 실시간으로 확인한 후 특정 주기로 대기열에 등록된 사용자를 참가열로 n명씩 이동시켜 참가열에 이동하게 되면 타겟 웹페이지로 자동으로 이동하는 방식으로 흐름이 바뀌도록 함

**대기열에 필요한 기능**

1. 들어오는 요청을 순서대로 저장
2. 각 사용자는 실시간으로 자신의 순위가 몇 번인지 알 수 있어야 함
...
각각의 접속을 key로 userId, score로 요청 시간으로 관리하며, 이를 통해 선착순 입장을 보장

또한 Sorted Set는 score 값을 이용하여 자동 정렬을 해주고 중복 삽입을 방지 해주며, 상위 N명을 추출하거나 제거하고, 특정 사용자의 순위를 O( logN )으로 반환하는 등의 명령어를 제공 

즉, Sorted Set을 사용하면 대기열에서 사용자의 순위를 정확하게 파악할 수 있을 뿐만 아니라, 예약 가능한 수량에 따라 상위 N명을 선별하여 예약을 확정하고, 나머지 인원은 대기 상태로 유지하거나 제거하는 로직을 손쉽게 구현할 수 있습니다.<br><br>
...
따라서 어떤 방식을 선택하는 것이 가장 좋을까 생각을 해보니 웹 소켓은 이미 한 번 경험 해보았기 때문에 경험해보지 못했던 Polling 방식이나 SSE 방식을 사용해볼까 합니다. <br><br>

### 흐름 구성

---

1. 대기열 등록 요청 → 이때 해당 클라이언트는 SSE를 통해 서버와 지속적인 스트림 연결을 맺습니다. 이후 이를 통해 실시간으로 자신의 대기 상태나 순번이 변경되면 변경된 정보 값을 전달받습니다.
2. Redis 대기열 Zset에 사용자를 추가 후 outbox 테이블에 해당 사용자를 저장합니다. ( 상태 값 : “WAIT” )
3. 사용자 등록, 삭제, 스케줄러에 의한 허용열 이동 등의 이벤트가 발생할 때마다 해당 사용자의 Outbox 상태 값이 변경됩니다
4. Outbox 테이블이 변경되어 binary 로그 값이 변경되면 Debezium 커넥터가 이를 감지하여  

5. 
6. 스케줄러가 주기적으로 대기열의 사용자를 참가열로 이동시키며, outbox 테이블의 이동한 사용자 상태 값을 "ALLOW"로 수정합니다.
7. Debezium이 outbox 테이블의 변경사항을 실시간으로 감지하여 Kafka에 이벤트를 발행하고, 서버는 Kafka 메시지를 수신한 뒤, SSE를 통해 클라이언트로 전달합니다.
8. 이 이벤트를 수신한 각 사용자는 자신의 현재 상태를 서버로부터 받음, 아직 대기열에 있는 사용자는 현재 순위를 실시간으로 전달받고, 참가열로 이동된 사용자는 "confirmed" 이벤트를 받아 타겟 페이지로 이동하게 되는 구조

### 문제 상황
만약 이벤트가 소실되어 특정 사용자가 타겟 페이지로 이동하지 못하는 상황이 발생한다면, 이는 서비스에 직접적인 영향을 줄 수 있습니다. 기존에는 Redis를 활용해 이벤트를 처리했지만, 이 방식에서는 이벤트 소실 시 재전송이 불가능한 구조라고 생각했습니다.

이 문제를 해결하기 위해, Kafka를 기반으로 이벤트를 전송하도록 구조를 개선하였고, 데이터베이스의 사용자 상태값과 이벤트 간 정합성을 비교하여 불일치가 감지될 경우 재시도를 동작하도록 변경하였습니다.

이 과정에서 Debezium MySQL 커넥터를 활용하여 MySQL 테이블에 변경이 발생하면 해당 변경 사항을 감지하고, Kafka 토픽으로 이벤트를 발행하도록 구현했습니다. 이를 통해 데이터베이스의 변경과 이벤트 발행 간의 트랜잭션 정합성을 확보하였습니다.

또한, 이벤트 발행 실패 시에도 데이터 손실 없이 재시도를 할 수 있으며, Debezium은 MySQL의 binlog를 기반으로 변경 사항을 순차적으로 스트리밍하기 때문에 이벤트의 순서도 자연스럽게 보장할 수 있습니다.

결과적으로, Transactional Outbox Pattern과 Debezium을 결합한 구조를 통해 이벤트 소실 방지, 이벤트 순서 보장, 데이터 정합성 확보를 효과적으로 해결할 수 있었습니다.
